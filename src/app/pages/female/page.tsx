...
async function getData() {
  const res = await fetch('https://u4m48ujb.api.sanity.io/v2021-10-21/data/query/production?query=*%5B_type%20%3D%3D%20%22product%22%20%26%26%20isForFemales%20%3D%3D%20true%5D%7B%0A%20%20%22firstImage%22%3A%20images%5B0%5D.asset._ref%2C%0A%20%20%20%20title%2C%20subTitle%2C%20price%2C%20_id%0A%7D%0A',
    {next: {revalidate: 600}});
      ...
      <div className={'flex flex-row justify-around gap-x-3 gap-y-7 flex-wrap w-full'}>

        {
          data.result.map((product: any) => {
            return (
              <div className={'cursor-pointer min-w-md flex flex-col gap-2 hover:scale-110 transition-all duration-300'} key={product._id}>
